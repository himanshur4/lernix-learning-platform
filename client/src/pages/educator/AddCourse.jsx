...
const AddCourse = () => {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0 bg-gradient-to-b from-fuchsia-100 opacity-90'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-700 bg-white p-4 rounded-md border border-fuchsia-700'>
        ...
        <div className="flex items-center justify-between flex-wrap">
          ...
          <div className="flex md:flex-row flex-col items-center gap-3">
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              <img src={assets.file_upload_icon} alt="" className='p-3 bg-fuchsia-700 rounded cursor-pointer' style={{ filter: 'invert(20%) sepia(100%) saturate(5000%) hue-rotate(300deg) brightness(100%) contrast(300%)' }}/>
        ...
        <div>
                ...
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                      <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a href={lecture.lectureUrl} target='_blank' className='text-fuchsia-500'>Link</a> - {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}</span>
          ...
          <div className='flex justify-center items-center text-white bg-fuchsia-600 p-2 rounded-lg cursor-pointer' onClick={()=>handleChapter('add')}>+ Add Chapter</div>
          {showPopup && (
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                ...
                <button type='button' className='w-full bg-fuchsia-500 text-white px-4 py-2 rounded' onClick={addLectures}>Add</button>
